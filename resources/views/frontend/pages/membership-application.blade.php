...
<section id="membership-application" class="py-24 lg:py-32 relative overflow-hidden">
    ...
    <div class="container mx-auto px-6 relative z-10">
        <div class="text-center max-w-3xl mx-auto mb-16 reveal-up">
            <span class="section-caption">Join Our Community</span>
            <h1 class="section-title">Membership Application</h1>
            <p class="section-text">
                Complete the form below to apply for membership at The Bengal Club. Our team will review your application and contact you shortly.
            </p>
        </div>
        ...
        <div class="max-w-5xl mx-auto">
            <div class="bg-dark-700 border border-accent/20 rounded-2xl p-8 lg:p-12 reveal-up">
                ...
                <form id="membership-application-form" class="space-y-8">
                    @csrf
                    ...
                    <div id="step-1">
                        ...
                        <div class="mb-8">
                            <h3 class="text-2xl font-bold text-white mb-6 flex items-center">
                                ...
                                Membership Category <span class="text-red-500">*</span>
                            </h3>

                            <div class="membership-options-grid grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                                @foreach($membershipCategories as $category)
                                    <label class="membership-radio-card relative cursor-pointer">
                                        <input type="radio" name="membership_category_id"
                                               value="{{ $category->id }}"
                                               class="peer sr-only"
                                               {{ isset($selectedCategoryId) && $selectedCategoryId == $category->id ? 'checked' : '' }}
                                               required>
                                        <div class="radio-card-content h-full p-6 bg-dark-800 border-2 border-accent/20 rounded-xl transition-all duration-300 peer-checked:border-accent peer-checked:bg-accent/5 hover:border-accent/40 hover:shadow-lg">
                                            <div class="flex items-start justify-between mb-3">
                                                <div class="radio-circle w-5 h-5 rounded-full border-2 border-accent/40 peer-checked:border-accent peer-checked:bg-accent transition-all flex items-center justify-center">
                                                    <div class="w-2 h-2 rounded-full bg-white opacity-0 peer-checked:opacity-100"></div>
                                                </div>
                                                @if($category->badge_text)
                                                    <span class="badge px-2 py-1 bg-accent/20 text-accent text-xs font-semibold rounded">{{ $category->badge_text }}</span>
                                                @endif
                                            </div>
                                            <div class="membership-details">
                                                <h4 class="text-xl font-bold text-white mb-2">{{ $category->title }}</h4>
                                                @if($category->bio)
                                                    <p class="text-sm text-gray-400 mb-3 line-clamp-2">{{ $category->bio }}</p>
                                                @endif
                                                <div class="price-duration mt-4 pt-4 border-t border-accent/10">
                                                    <div class="flex items-baseline justify-between">
                                                        <span class="price text-2xl font-bold text-accent">৳{{ number_format($category->price, 2) }}</span>
                                                        <span class="duration text-sm text-gray-400 bg-dark-900 px-3 py-1 rounded-full">{{ $category->duration }}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <span class="error-message text-red-500 text-sm mt-2 hidden" id="category-error"></span>
                        </div>
                    ...
                    <div id="step-2" class="hidden space-y-8">
                        ...
                        <div class="bg-accent/10 border border-accent/30 rounded-2xl p-6 text-center">
                            ...
                            <p class="text-5xl font-extrabold text-accent mb-2">
                                ৳{{ $siteSetting->application_fee ? number_format($siteSetting->application_fee, 2) : '—' }}
                            </p>
